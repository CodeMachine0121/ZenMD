...
1. **送一筆測試訂閱，確認欄位有進去。** Buttondown 已經接好了
   ...
   **表單一定要正常導頁，不可以送進隱藏 iframe。**

   Buttondown 收到內嵌訂閱之後，回的是一個**帶 CAPTCHA 的驗證頁**
   （「Verify and Subscribe」），**必須由使用者親自點一下才算訂閱**。
   帶不帶瀏覽器標頭都一樣會出現，不是防機器人的例外處理。

   > ⚠️ 這一頁曾經把表單送進看不見的 iframe，想讓使用者留在原地。
   > 結果是驗證頁被藏在 iframe 裡沒人點得到，**而畫面還顯示「收到了」**——
   > 名單永遠是空的，你卻會以為表單好了。
   > **沒訂閱成功不可怕，宣稱訂閱成功才可怕**：你會分不出是沒人要，還是東西壞了。

   所以現在按下送出會跳到 Buttondown 的頁面，文案也先講清楚了。
   如果 Buttondown 後台可以關掉那個驗證步驟，關掉之後體驗會好一些，
   但**在確認關得掉之前，導頁是唯一保證會成功的做法**。
