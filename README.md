...
1. **接上 Buttondown。** `index.html` 裡的
   `REPLACE_WITH_YOUR_BUTTONDOWN_USERNAME` 換成你的帳號名稱。

   最保險的做法：**去 Buttondown 後台複製它給的內嵌表單網址，整段 `action` 貼過來**，
   不要相信原始碼裡拼出來的那串（服務換過網域，格式也可能再改）。

   然後**自己送一筆測試**，到後台確認兩件事：

   | 檢查 | 沒過的話 |
   | :--- | :--- |
   | 訂閱者身上有 `wants_cloud` 這個 metadata | 改用 tag：把勾選框的 `name` 換成 `tag`、`value` 換成 `wants-cloud` |
   | 標籤 `landing` 有帶上 | 之後若在別處也放表單，這個標籤是用來分流量來源的 |
